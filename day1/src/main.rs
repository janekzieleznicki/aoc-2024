...
fn main() {
    ...
    let calc_dist = total_distance(&mut BufReader::new(
        File::open(example_data.clone()).unwrap(),
    ));
...
where Reader: ?Sized + BufRead{
    ...
    right
        .into_iter()
        .for_each(|r| *occurrances.entry(r).or_default() += 1);
    left.iter()
...
mod tests {
    ...
    fn test_distance() {
        ...
        assert_eq!(
            11,
            total_distance(&mut BufReader::new(File::open(example_data).unwrap()))
        );
    }
    #[test]
    fn test_similarity_score() {
        ...
        assert_eq!(
            31,
            similarity_score(&mut BufReader::new(File::open(example_data).unwrap()))
        );
    }
}
